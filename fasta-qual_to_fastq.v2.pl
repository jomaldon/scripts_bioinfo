...
while (my $fastarec = <FASTA>) {
  ...
  if($ls > $lq){
    $seq = substr $seq, 0, $lq;
    my $lnew = length($seq);
    print STDERR "length missmatch: $fid fasta=$ls qual=$lq newseq=$lnew\n";
  }
  if($ls < $lq){
    $quals = substr $quals, 0, $ls;
    my $lnew = length($quals);
    print STDERR "length missmatch: $fid fasta=$ls qual=$lq newqual=$lnew\n";
  }
